...
| `transition-edge.McpSession.initializing.failed`, `rule-success.SessionInitFailed`/`.failure.1` | recorded gap (G2) | init-failure is delegated: a not-ready/unreachable device makes `device::connect` bail in `main` **before** any MCP session exists. (rmcp itself never rejects on version — `ProtocolVersion` has total lexical order, so it down-negotiates *any* client string to the common floor; the `UnsupportedProtocolVersion` path is dead.) No bridge-level `failed` session object/test — recorded intentional gap (PHASE5.md weed log) |
