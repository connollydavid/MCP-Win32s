...
|---|---|---|
| `entity-fields.CatalogListing` | unit + wire | `CatalogListing` is a spec-model abstraction (no standalone C struct — the device serialises the response directly), discharged at the wire shape: `entry_count` by `test_catalog.c` `listing_reflects_loaded_catalog`, the `request_id` round-trip by `test_serial.c` `list_commands_round_trip`, the serialised entry array by `test_catalog.c` `list_commands_reports_loaded` |
| `entity-relationship.Catalog.entries` | unit | `test_catalog.c` `list_commands_reports_loaded` + `listing_reflects_loaded_catalog`: the serialiser reads exactly the loaded `CatalogEntry` set (the navigable relation the listing consumes), emitting each as `{name, builtin, destructive, flags}`, and the emitted count equals the catalog's loaded `entry_count` |
