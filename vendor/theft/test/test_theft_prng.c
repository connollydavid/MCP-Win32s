...
SUITE(prng) {
    ...
    volatile uint64_t limit64;
    RUN_TEST(prng_should_return_same_series_from_same_seeds);

    for (limit = 100; limit < 100000; limit *= 10) {
    ...
    for (limit64 = 1; limit64 < 300; limit64++) {
        RUN_TESTp(check_random_choice_distribution__slow, limit64, 0.05);
    }
